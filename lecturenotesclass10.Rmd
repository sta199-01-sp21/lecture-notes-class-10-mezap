...
```{r packages}
library(usethis)
use_git_config(user.name = "mezap", user.email = "alejandro.meza@duke.edu")
library(tidyverse)
...
pretty close- 3/8 = 0.375 ~= 0.3778 and
1/8 = 0.125 ~= 0.1243
...
-Its the sum of the probabilities of getting both red, both white, both blue.  
...
-My guess: The latter of the two.

```{r simulate_rolls}
rolls <- c(1, 2, 3, 4, 5, 6)
two_rolls_results <- tibble(
  roll_1 = sample(rolls, size = 10000, replace = TRUE),
  roll_2 = sample(rolls, size = 10000, replace = TRUE)
)
two_rolls_results
```

```{r identify_events}
two_rolls_results <- two_rolls_results %>% 
  mutate(event1 = sqrt(roll_1) == roll_2,
         event2 = abs(roll_1-roll_2) == 4)
two_rolls_results
```
